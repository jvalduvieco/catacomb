...
-export([init/1, handle_call/3,handle_cast/2,terminate/2,code_change/3,handle_info/2,get_game_info/1]).
...
login(SessionPid,User,Password) ->
  gen_server:call(SessionPid, {login, [User, Password]}).
set_character(SessionPid,CharacterId) ->
  gen_server:call(SessionPid,{set_character,CharacterId}).
get_game_info(SessionPid) ->
  gen_server:call(SessionPid,{get_game_info}).
%% User Callbacks
...
handle_call({get_game_info},_From, State) ->
  GameInfo=[{"heartbeat_enabled",ct_config_service:lookup(heartbeat_enabled)}],
  {reply,GameInfo,State}.
